...
```
/plugin marketplace add https://github.com/eskyzr/claudecode-plugins.git
/plugin install bedrock-limiter@eskyzr
```
...
```bash
PLUGIN_ROOT=~/.claude/plugins/cache/eskyzr/bedrock-limiter/0.1.0
...
`go/` ディレクトリ配下に Go ソースと Makefile があります。
...
# または公式バイナリ
curl -fsSL https://go.dev/dl/go1.26.2.linux-amd64.tar.gz | sudo tar -C /usr/local -xz
...
全プラットフォーム向けにクロスコンパイルされ `go/bin/` に出力されます。

| バイナリ | 対象 |
|---|---|
| `bedrock-limiter-linux-amd64` | Linux x86_64 |
| `bedrock-limiter-linux-arm64` | Linux ARM64 |
| `bedrock-limiter-darwin-amd64` | macOS Intel |
| `bedrock-limiter-darwin-arm64` | macOS Apple Silicon |
| `bedrock-limiter-windows-amd64.exe` | Windows x64 |

ビルド後はバイナリをコミットしてプッシュしてください。
