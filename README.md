...
```bash
python3 ~/.claude/plugins/installed/bedrock-limiter/hooks/bedrock_limiter.py --update-pricing
```
...
```bash
# コマンドラインから確認
python3 <plugin-root>/hooks/bedrock_limiter.py --status

# 料金キャッシュを手動更新
python3 <plugin-root>/hooks/bedrock_limiter.py --update-pricing
```
