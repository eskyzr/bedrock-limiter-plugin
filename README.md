...
● UserPromptSubmit operation blocked by hook:                                                  
  [node ${CLAUDE_PLUGIN_ROOT}/hooks/bedrock_limiter.js]: ⛔ 日次コスト上限超過:             
  $3.9745 / $3.00                                                                              
...
Node.js（Claude Code が動いている環境には必ずインストール済み）
...
```bash
node ~/.claude/plugins/cache/ie/bedrock-limiter/1.0.0/hooks/bedrock_limiter.js --update-pricing
```
...
# コマンドラインから確認
node $PLUGIN_ROOT/hooks/bedrock_limiter.js --status

# 料金キャッシュを手動更新
node $PLUGIN_ROOT/hooks/bedrock_limiter.js --update-pricing
```
